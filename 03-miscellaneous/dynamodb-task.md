...
```
GSI1PK = <date>
GSI1SK = MEAL#<meal_type>#<userID>
```

GSI1 exists for cross-user headcount by date. Including `meal_type` in `GSI1SK` lets logistics query a specific meal type directly. For example, querying `GSI1PK = <date>` with `GSI1SK begins_with "MEAL#lunch#"` returns all lunch participation records for that date across all users in one call.
...
```
1. `PK = WFHPERIOD` + `SK = <startDate>#<endDate>`
2. `PK = WFHPERIOD` + `SK begins_with <startDate>`
```
