...
import type { DayStatus, MealType } from "../../types";
...
export const CreateScheduleModal: React.FC<CreateScheduleModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // For office_closed days, meals are not required
        if (formData.dayStatus !== "office_closed" && formData.dayStatus !== "govt_holiday" && formData.availableMeals.length === 0) {
        ...
        try {
            const { createSchedule } = await import("../../services/scheduleService");
            ...
            const msg = error?.response?.data?.error?.message || "Failed to create schedule";
            toast.error(msg);
        } finally {
    ...
    return (
        ...
        >
            <div className="bg-[#FFFDF5] rounded-[2rem] shadow-[var(--shadow-clay)] max-w-lg w-full p-8 border border-white/60 relative overflow-hidden">
                {/* Header */}
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-2xl font-black text-[#23170f]">Create Schedule</h2>
                    <button
                        type="button"
                        onClick={handleClose}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative group">
                            <input
                                ...
                                className="w-full bg-[var(--color-background-light)] text-[#23170f] rounded-2xl border-none shadow-[var(--shadow-clay-inset)] px-4 py-3 focus:ring-2 focus:ring-[var(--color-primary)]/50 transition-all cursor-pointer font-medium"
                                required
                            />
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            <select
                                ...
                                onChange={(e) => setFormData({ ...formData, dayStatus: e.target.value as DayStatus })}
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-bold text-[var(--color-text-sub)] uppercase tracking-wider ml-1">
                            Reason / Notes
                            <span className="text-xs normal-case text-[var(--color-text-sub)]/60 ml-1">(Optional for normal days)</span>
                        </label>
                        <textarea
                            ...
                            {mealOptions.map((meal) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-lg bg-${meal.color}-100 flex items-center justify-center text-${meal.color}-600`}>
